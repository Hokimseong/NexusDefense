...
{
    public GameObject arrowPrefab; // 화살표 프리팹 (Canvas 안에 있어야 함)
    public Transform player; // 플레이어 위치
    public Camera mainCamera;
    private List<Transform> enemies = new List<Transform>(); // 현재 존재하는 적들
    private Dictionary<Transform, RectTransform> enemyArrows = new Dictionary<Transform, RectTransform>(); // 적당 화살표 매핑

    void Start()
    {
        InvokeRepeating(nameof(UpdateEnemyList), 0f, 1f); // 1초마다 적 목록 업데이트
    }
    ...
    void Update()
    {
        ...
        float offset = 50f; // 화면 가장자리에서 약간 안쪽으로 배치
